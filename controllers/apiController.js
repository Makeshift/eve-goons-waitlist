...
exports.fleetAtAGlance = function(req, res) {
    ...
    fleets.get(req.params.fleetid, function (fleet) {
        ...
        <tbody>`
            ...
            html += `<tr>`
            var count = 1;
            for( keyof in fleetArray) {
                html += `<td class="tw35"><img src="https://image.eveonline.com/Render/`+fleetArray[keyof].ship_type_id+`_32.png" alt="Ship Icon"></td>
                  <td class="tw20per"><a href="#">`+fleetArray[keyof].ship_type_name+`</a></td>
                  <td>`+fleetArray[keyof].count+`</td>`
                if (count % 3 === 0) {
                    html += `</tr>
                    <tr>`
                }
